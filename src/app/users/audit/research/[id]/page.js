...
import { Button, Input, Select, Card, Descriptions, Tag, Modal } from 'antd';
...
const ResearchList = ({ params }) => {
    ...
    useEffect(() => {
        fetchResearch(Number(params.id));
    }, [params.id]);
    ...
    const fetchResearch = async (id) => {
        ...
        try {
            const res = await fetch(`/api/auditResearch/${id}`);
            if (!res.ok) {
        ...
        EvaluationAlert('ยืนยันการประเมิน', 'คุณแน่ใจหรือไม่ที่จะทำการประเมินผลงานนี้?')
            .then(async (result) => {
                if (result.isConfirmed) {
                    try {
                        const response = await fetch(`/api/auditResearch/${id}`, {
                            method: 'PUT',
                            headers: {
    ...
    const filteredResearch = Array.isArray(research) ? research.filter((research) => {
        return (
            (selectTerm === 'all' || auditStatus[research.audit] === selectTerm) &&
            (
    ...
    return (
        <div className="max-w-6xl mx-auto px-4 mt-2">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold mb-6" style={{ color: '#2D427C' }}>ตรวจสอบผลงานวิจัย</h1>
                    ...
                    filteredResearch.map((research, index) => (
                        ...
                        >
                            <Descriptions layout="horizontal" size="small" className="small-descriptions">
                                ...
                                <Descriptions.Item label="สถานะ"><Tag color={StatusColors[research.audit]}>{auditStatus[research.audit]}</Tag></Descriptions.Item>
